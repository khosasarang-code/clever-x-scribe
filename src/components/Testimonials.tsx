...
export function Testimonials() {
  ...
  const [baseCount, setBaseCount] = useState(12458);
  useEffect(() => {
    // Steady drift forward from a fixed anchor so the number always reads 12,458+
    const anchor = new Date("2025-01-01T00:00:00Z").getTime();
    const elapsedSec = Math.max(0, (Date.now() - anchor) / 1000);
    setBaseCount(12458 + Math.floor(elapsedSec * 0.08));
      ...
      <div className="mx-auto max-w-2xl px-4">
        <div className="relative rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-card/40 to-accent/10 backdrop-blur p-6 sm:p-8 text-center overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight tabular-nums text-gradient-brand min-h-[1em]">
              {counterReady ? `${liveCount.toLocaleString()}+` : "\u00A0"}
            </div>
            <div className="text-sm sm:text-base text-muted-foreground mt-2">
              smart replies generated
            </div>
          </div>
        </div>
      </div>
